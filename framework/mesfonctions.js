...
module.exports.genererNomFichier = async (extension) => {
    ...
    var nomFichier = `spilux${randomNbre}.${extension}`;
    return nomFichier;
};
/** ****** */
/** ************ */
...
async function ajouterCommande() {
    fs.readdirSync(__dirname + "/../spiluxplugins").forEach((fichier) => {
        if (path.extname(fichier).toLowerCase() == ".js") {
            require(__dirname + "/../spiluxplugins/" + fichier.split(".js")[0]);
...
async function xlab() {
    ...
    var chemin = './spiluxplugins/';
    var nomFichier = await readDir(chemin);
    nomFichier.forEach((fichier) => {
        if (fichier.endsWith(".js")) {
            var { commande } = require(__dirname + '/../spiluxplugins/' + fichier.split(".js")[0]);
            var infos;
            if (commande) {
                infos = commande();
            }
