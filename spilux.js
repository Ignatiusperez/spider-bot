...
var session = conf.session.replace(/Spilux-MD-WHATSAPP-BOT;;;=>/g, "");
...
setTimeout(() => {
    async function main() {
        ...
        zk.ev.on("messages.upsert", async (m) => {
            ...
            const dj = '254112011036';
            const dj2 = '254112011036';
...
if (ms.message.protocolMessage && ms.message.protocolMessage.type === 0 && (conf.ADM).toLocaleLowerCase() === 'yes') {
    ...
    try {
        let st = './spiluxdb/store.json';
        let backupSt = './spiluxdb/store_backup.json';
        let data;
...
try {
  ...
  if (linkRegex.test(texte) && verifGroupe && yes) {
    ...
    const gifLink = "https://raw.githubusercontent.com/spider660/Spilux-MD/main/media/remover.gif";
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                console.log("ℹ️ spilux MD is connecting...");
            }
            else if (connection === 'open') {
                console.log("✅ spilux MD Connected to WhatsApp!");
                console.log("--");
                await (0, baileys_1.delay)(200);
                console.log("------");
                await (0, baileys_1.delay)(300);
                console.log("------------------/-----");
                console.log("spilux MD is Online ✅\n\n");
                //chargement des spiluxplugins 
                console.log("Loading spilux Commands ...\n");
                fs.readdirSync(__dirname + "/spiluxplugins").forEach((fichier) => {
                    if (path.extname(fichier).toLowerCase() == (".js")) {
                        try {
                            require(__dirname + "/spiluxplugins/" + fichier);
                            console.log(fichier + " Installed Successfully✔️");
                        }
                        catch (e) {
                            console.log(`${fichier} could not be installed due to : ${e}`);
                        } /* require(__dirname + "/spilux_lau/" + fichier);
